...
// console.log("matchesData" ,matchesData);
    ...
    matches.forEach((match, index) => {
        ...
        const rawData = sessionStorage.getItem('Probability Data');
        const winning_player_odds = rawData ? JSON.parse(rawData) : null;

        
       
if (winning_player_odds && Array.isArray(winning_player_odds)) {
    // Loop through the array directly since data isn't nested
    winning_player_odds.forEach((playerData, dataIndex) => {
        if (dataIndex === index) {
            ...
            const formattedPlayerWinProb = (typeof playerWinProb === 'number') 
                ? playerWinProb.toFixed(2) 
                : playerWinProb;
    ...
    // var amount_target =""
       ...
       const amountInput = document.createElement('input');
        ...
        runnersCell.textContent = matches1[index];
            ...
            strategies.forEach(strategy => {
                ...
                // Rest of your existing code...
                    ...
                    if (checkbox.checked) {
                        ...
                        if (ind !== -1) {
                            all_data.data.market_catalogue[ind].strategies = matchStrategy;
                            ...
                            if (selectedRow) {
                                ...
                                if (strategyCell && profitCell) {
                                    strategyCell.textContent = strategyDisplayNames[matchStrategy];
            ...
            matchStrategy = 'strategy_3';
            dropdownIcon.textContent = strategyDisplayNames[matchStrategy];
            
            const listItem = document.createElement('li');
            listItem.textContent = strategyDisplayNames[matchStrategy];
            listItem.style.cursor = 'pointer';
            listItem.style.padding = '5px';
            listItem.style.fontSize = '15px';
            
            // Add message box for Strategy 3
            const messageBox = document.createElement('div');
            const infoIcon = document.createElement('i');
            infoIcon.classList.add('fa-solid', 'fa-circle-info');
            infoIcon.style.marginLeft = '10px';
            ...
            messageBox.innerHTML = '<b>Hybrid Strategy</b><br>' +
                'This approach enhances hedging strategy by adjusting stakes on Back and Lay bets to guarantee profit, ' +
                'regardless of the outcome. <br>' +
                '<b>Unmatched Risk:</b> Moderate, There are chances of Unmatched Lay Bet.<br>' +
                '<b>Minimum Amount:</b> $3 per match.<br>' +
            ...
            // Update the match data with Strategy 3
            matches11[index].selectedStrategy = matchStrategy;
            matches11[index]["gp_strat"] = matchStrategy;
